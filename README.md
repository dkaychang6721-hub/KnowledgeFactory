...
| `Agent.chat() got an unexpected keyword argument 'files'` | `google-antigravity` 套件改版後，不再支援用 `files=[...]` 參數夾帶音訊檔，必須改用 `google.antigravity.types.Audio` 物件 | `podcast_agent.py` 已改為讀取音訊位元組建立 `Audio(data=..., mime_type="audio/mpeg")`，並把它跟文字提示一起包成 list 傳給 `agent.chat()` |
...
| v1.5 | 修正 `.bat` 在繁體中文 Windows 下因 `chcp 65001` + 中文字搭配出現亂碼、指令找不到的問題（改成純 ASCII，移除 chcp）；修正 `google-antigravity` 套件改版後 `Agent.chat()` 不再支援 `files=[...]` 參數的錯誤（改用 `Audio` 物件） |
